"use strict";
	...
	function registrate() {
		document.getElementById('error').innerHTML = '';
		const err = document.getElementById("errorText");
		const account = document.registrate.account.value.trim();
		if (account === "") {
			err.innerHTML = "Login is empty";
			return;
		}
		const password = document.registrate.password.value;
		const confirmPassword = document.registrate.confirmPassword.value;
